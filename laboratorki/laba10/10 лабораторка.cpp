/*1)Средства изменения длины : Чтобы изменить длину вектора, вы можете использовать
функции resize, push_back и pop_back.Функция resize принимает размер в качестве аргумента
и может увеличить или уменьшить размер вектора.Функция push_back добавляет новый элемент в
конец вектора, а функция pop_back удаляет последний элемент из вектора.
...
int main() {
    ...
    cout << "You give me " << numbers.size() << " elements in vector" << endl;
    //пункт 2
